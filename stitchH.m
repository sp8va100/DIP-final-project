function result = stitchH(img_src, H, img_des)
    ...
    result_src = imtransform(img_src, maketform('projective', H'), 'XData', xdata_out, 'YData', ydata_out);
    result_des = imtransform(img_des, maketform('affine', eye(3)), 'XData', xdata_out, 'YData', ydata_out);

    result = result_src + result_des;
    overlap_pt = (result_src > 0.0) & (result_des > 0.0);
    result_avg = (result_src); % Note overflow
